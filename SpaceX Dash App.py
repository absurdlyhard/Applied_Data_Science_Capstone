...
def get_scatter_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title=f'All sites - payload mass between {int(payload_range[0]):8,d}kg and {int(payload_range[1]):8,d}kg'
                         )
    else:
        filtered_df = spacex_df[(spacex_df['Launch Site'] == entered_site) &
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title=f'Site {entered_site} - payload mass between {int(payload_range[0]):8,d}kg and {int(payload_range[1]):8,d}kg'
                         )
